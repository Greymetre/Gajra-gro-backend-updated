import { Controller, Get, Post, Body, Param, HttpCode, UsePipes, ValidationPipe , Req, UseInterceptors} from '@nestjs/common';
...
import { CreateOrderDTO, CreateCartItemsDto, GetQueryUserDto, CancelOrderDto } from 'src/dto/order-dto';
...
export class OrdersController {
  ...
  protected async createRedemption(@Req() req: Request, @Body() createOrderDTO: CreateOrderDTO ): Promise<any> {
    const authInfo = await getCustomerAuthInfo(req.headers)
    createOrderDTO.customerid = authInfo._id
  ...
  @ApiOperation({ summary: 'Add Cart Items' })
  @ApiResponse({ status: 200, description: 'Success', type: OrdersService })
  @ApiBadRequestResponse({ description: 'Invalid Cart Items' })
  @ApiInternalServerErrorResponse({ description: 'Internal server error' })
  @Post('addCartItems')
  @HttpCode(200)
  protected async AddCartItems(@Req() req: Request, @Body() cartItemsDTO: CreateCartItemsDto ): Promise<any> {
    const authInfo = await getCustomerAuthInfo(req.headers)
    cartItemsDTO.customerid = authInfo._id
      const data = await this.orderService.addCartItems(cartItemsDTO);
      return { data };
  };
